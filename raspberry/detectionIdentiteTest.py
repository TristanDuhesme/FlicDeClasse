...
subjects = {"1":"Tristan Duhesme", "2":"Farah Mouri", "3":"Remi Peltier","4":"Qianwen Bian","5":"Yanchi Lu", "6":"Mateo Vallouis", "7":"Sylvain Pascou", "8": "Adrien Chao", "9":"Ghislain Fournier", "10": "Chengxian Lin", "11":"Nicolas Turcant", "12": "Pierre Chauvet","13":"Tristan Martin","14":"MateoLunettes Vallouis","15":"Emmanuel Dervieux","16":"Alexandre Marmignon"}
...
def load_training_data(dataxml):
    face_recognizer = cv2.face.createLBPHFaceRecognizer()
    face_recognizer.load(dataxml)
    return face_recognizer

def prepare_training_data(tableLink):
    # ------STEP-1--------
    ...
    for nomAdresse in cursor.fetchall():
        print(str(nomAdresse))
     
    print('testAffichage 2:')   
    cursor.execute("SELECT adresse FROM {};".format(tableLink)) 
    for nomAdresse in cursor.fetchall():
        print(str(nomAdresse))
    
        #chercher nom du dossier dans la bdd
        # ------STEP-2--------
        ...
        chiffreBlanc= str(nomAdresse[0]).translate(trantab)
        print(chiffreBlanc)
        chiffre=chiffreBlanc.strip()
        print(chiffre)
        label = int(chiffre)
        #label = int(nomAdresse.replace("/home/pi/Raspberry-Projects/training-data/s", ""))
        #label = int(nomAdresse[0].translate(str.maketrans({"/": "", "h": "", "o": "","m": "", "e": "", "p": "","i": "", "R": "", "a": "","s": "", "b": "", "r": "","y": "", "-": "", "P": "","j": "", "c": "", "t": "","n": "", "g": "","d": "", "a": "", "t": "" })))
        #label = dir_name
        # build path of directory containing images for current subject subject
        # sample subject_dir_path = "training-data/s1"
        subject_dir_path = nomAdresse[0]
        #a changer 
        # the images names that are inside the given subject directory
        ...
        for image_name in subject_images_names:
            ...
            if face is not None:
                ...
                faces.append(face)
                #cv2.imshow('face detectee', cv2.resize(face, (400, 500)))                # add label for this face
                labels.append(label)
...
def initialiserDonnees(table):
    print("Preparing data...")
    faces, labels = prepare_training_data(table)
    print("Data prepared")
...
def predict(face_recognizer, test_img):
    ...
    if (face==None):
        print("Attention : Pas de tete detectee")
        return img, None
    print(face)
...
def identificationImage(recognizer, cheminImage):
    
    #print("Predicting images...")

    # load test images
    test_img = cv2.imread(cheminImage)
    # perform a prediction
    #'/home/pi/Raspberry-Projects/tristanTest.jpg'
    predicted_img, identite = predict(recognizer, test_img)
    #print("Prediction complete")
    return predicted_img, identite
    
def afficherImageIdentifie(predicted_img):
    cv2.imshow('personne1', cv2.resize(predicted_img, (400, 500)))
#    cv2.waitKey(0)
 #   cv2.destroyAllWindows()
  #  cv2.waitKey(1)
   # cv2.destroyAllWindows()
...
#face_recognizer = initialiserDonnees('/home/pi/Raspberry-Projects/training-data')
#imagePredie = identificationImage(face_recognizer, '/home/pi/Raspberry-Projects/tristanTest.jpg')
#imagePredie3 = identificationImage(face_recognizer, '/home/pi/Raspberry-Projects/tristanTest.jpg')
#imagePredie2 = identificationImage(face_recognizer, '/home/pi/Raspberry-Projects/remiTest2.jpg')

#afficherImageIdentifie(imagePredie)
#afficherImageIdentifie(imagePredie3)
#afficherImageIdentifie(imagePredie2)
